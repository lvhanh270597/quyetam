...
<div class="card card-cascade">
  ...
  <div class="view view-cascade gradient-card-header blue-gradient">    
    ...
    <?php 
        $user = $this->user_ml->get_by_primary($guess);
        echo $user['full_name'];
    ?>
    </h2>    
  </div>
      ...
      <div class="inbox_msg">
        <div class="inbox_people">
          ...
          <div class="inbox_chat">
            <?php
            if (!isset($all_last) || count($all_last) == 0){
                echo 'No message';
            } else {                
                foreach ($all_last as $one){                    
                    $person_id = get_another($one['from_user'], $one['to_user'], $this->my_username);
                    $person = $this->user_ml->get_by_primary($person_id);
                    $active = ($person_id == $guess) ? ' active_chat' : '';                        
                    $open_strong = $one['seen'] ? '' : '<strong>';
                    $close_strong = $one['seen'] ? '' : '</strong>';
                    echo '
                    <a href="'.site_url('message/t/'.$person_id).'"> <div class="chat_list'.$active.'" id="'.$person_id.'">
                        <div class="chat_people">
                            <div class="chat_img"> <img src="'.$images[$person_id].'" alt="sunil"> </div>
                            <div class="chat_ib">
                            '.$open_strong.'<h5 class="fuck">'.$person['full_name'].'</h5><h5><span class="chat_date">'.$one['time'].'</span></h5>
                            <p>'.$one['content'].'</p>'.$close_strong.'
                            </div>
                        </div>
                    </div> </a>                  
                    ';
        ...
        <div class="type_msg">
            <div class="input_msg_write">
              <textarea type="text" class="write_msg" placeholder="Type a message" name="content"/></textarea>
              <button class="msg_send_btn" type="submit" id="send_message">
...
<script>
    $("textarea[name='content']").on('keypress', function (e) {
         if(e.which === 13){
            ...
            $.ajax({
            url:<?php echo '\''.base_url('message/ajaxRequestPost/'.$user['username']).'\',';  ?>
            type: 'POST',
